...
public class RESTAssuredBase extends PreAndTest {
	...
	public static Integer extractResponseContentforInt(Response response, String key) {

		JsonPath jsonPath = response.jsonPath();
		Integer actValueInt = jsonPath.get(key);
		return actValueInt;
	...
	public static Response postWithBodyAsidandurl(String bookingid, String jsonObject, String URL) {
		return setLogs().header("BookingID",bookingid).body(jsonObject).post(URL);
	}

	public Response putWithBodyAndAuth(String Auth, String jsonObject, String URL) {
		return RestAssured.given().header("Authorization",Auth).contentType(getContentType()).request().body(jsonObject).when().put(URL);
	}
